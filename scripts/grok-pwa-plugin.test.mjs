...
// An empty workspace. The plugin's cwd defaults to process.cwd(), so tests
// without an explicit `site`/`cwd` would read this repo's real public/og.jpg
// and site.json (the app's own branding) instead of the bare-template
// behavior they pin.
const isolatedCwd = () => mkdtempSync(join(tmpdir(), "grok-og-iso-"));
